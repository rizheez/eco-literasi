...
export const Internalisasi: React.FC = () => {
  ...
  const handleSelectChoice = (choiceIdx: number) => {
    ...
    if (choice.isCorrect) {
      ...
      speakIndonesian(`Hebat! ${choice.outcomeTitle}. ${choice.outcomeDesc}. Sekarang mari berjanji untuk menyayangi bumi.`);
    } else {
      playSound('error');
      speakIndonesian(`Oh tidak. ${choice.outcomeTitle}. ${choice.outcomeDesc}. Coba pilih tindakan yang baik ya.`);
    }
  };
  ...
  const handleReflect = async () => {
    ...
    speakIndonesian("Terima kasih sudah berjanji! Kamu anak hebat pelindung lingkungan. Dua bintang ditambahkan!");
  };
    ...
    <div className="space-y-6 py-4">
      ...
      <div className="flex items-center space-x-3 bg-white/80 backdrop-blur-md p-4 rounded-3xl border-3 border-emerald-100/85 shadow-sm">
        {activeScenarioIdx === null ? (
          ...
          <button onClick={() => { playSound('click'); setActiveScenarioIdx(null); }} className="p-3 bg-white rounded-2xl border-2 border-emerald-100 hover:bg-emerald-50 transition text-slate-700 cursor-pointer shrink-0">
            <ChevronLeft size={24} />
          </button>
        )}
        <div>
          <h2 className="text-3xl font-extrabold text-emerald-800">Internalisasi Nilai Eco</h2>
          <p className="text-slate-600 font-bold text-sm">Pelajari hubungan sebab-akibat perilaku manusia terhadap kelestarian alam.</p>
        </div>
      </div>
      ...
      {activeScenarioIdx === null ? (
            ...
            return (
              ...
              >
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-slate-500 font-semibold text-sm">Simulasi interaktif pilihan anak</p>
                </div>
        ...
        <div className="bg-white rounded-3xl p-6 md:p-8 border-4 border-emerald-300 shadow-playful space-y-6">
          <div className="text-center space-y-2">
            <h3 className="text-2xl font-black text-emerald-900">{scenarios[activeScenarioIdx].title}</h3>
          ...
          <div className="flex justify-end pt-4">
            <button
              onClick={() => { playSound('click'); setActiveScenarioIdx(null); }}
